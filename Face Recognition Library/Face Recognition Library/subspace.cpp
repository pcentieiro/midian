...
void cv::subspace::LDA::lda(InputArray _src, InputArray _lbls) {
    ...
    vector<Mat> meanClass;
    int numClass[C];
    // initialize
    for (int i = 0; i < C; i++) {
        numClass[i] = 0;
        meanClass.push_back(Mat::zeros(1, D, data.type())); //! Dx1 image vector
    }
    // calculate sums
    for (int i = 0; i < N; i++) {
        ...
        add(meanClass.at(classIdx), instance, meanClass.at(classIdx));
        numClass[classIdx]++;
    }
    // calculate means
    ...
    for (int i = 0; i < C; i++)
        meanClass.at(i).convertTo(meanClass.at(i), meanClass.at(i).type(),
                1.0 / static_cast<double> (numClass[i]));
    // subtract class means
    for (int i = 0; i < N; i++) {
        int classIdx = mapped_labels[i];
        Mat instance = data.row(i);
        subtract(instance, meanClass.at(classIdx), instance);
    }
    ...
    for (int i = 0; i < C; i++) {
        Mat tmp;
        subtract(meanClass.at(i), meanTotal, tmp);
